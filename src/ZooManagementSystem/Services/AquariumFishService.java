...
import ZooManagementSystem.DomainModels.ClownFish;
import ZooManagementSystem.DomainModels.Colour;
import ZooManagementSystem.DomainModels.GoldFish;
import ZooManagementSystem.DomainModels.Pattern;
import ZooManagementSystem.Repositories.AnimalRepository;
import ZooManagementSystem.Utils.FishAttributeRandomizer;

public class AquariumFishService extends AnimalServices<AquariumFish> {
    public AquariumFishService(AnimalRepository repo) {
        super(repo);
    };
    ...
    public AquariumFish createNewFishByType(int type, int age, double length, List<Colour> singularFishColours,
            Pattern Pattern) {
    ...
    public List<AquariumFish> addRandomFish(int num) {
        for (int i = 0; i < num; i++) {
            ...
            if (typeOfFish == 1) {
                fish = new AquariumFish(ageTemp, lengthTemp, FishAttributeRandomizer.randomColour(),
                        FishAttributeRandomizer.randomPattern());
            }
            if (typeOfFish == 2) {
                List<Colour> colours = Arrays.asList(Colour.BLACK, Colour.GOLD, Colour.ORANGE, Colour.YELLOW);
                Colour colour = FishAttributeRandomizer.randomSingleColour(colours);
    ...
    private String find2MostPopularColour(Map<Colour, Integer> colourCounts, Colour mostCommonColour,
            Colour secondMostCommonColour,
